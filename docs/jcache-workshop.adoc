...
The `CommentsManager` is a request scoped CDI bean.
It has three business methods: `getAllComments()`, `submitComment()` and `deleteCommentById()`.
...
There's also a method that handsles POST requests (`login()`).
