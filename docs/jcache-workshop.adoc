...
* Use `@CacheDefaults` instead of injecting the cache
* Remove code that puts data in the cache and replace it with `@CachePut`, `@CacheKey` and `@CacheValue` annotations
* Remove code that reads data from the cache and replace it with `@CacheResult` and `@CacheKey` annotations
