...
Our first task is to enable JCache for our comments. Before doing that we need to be able to get a _javax.cache.Cache_ instance that will be used 
in our JCacheCommentsManager implementation. То do so create the CommentsCacheProducer cache provider class as follows:
...
Redeploy the application and chech that it is working with your new JCache provider.
...
Features to demonstrate (supporting JCache APIs):
 ...
 - using _CacheEntryListener_s to intercept Cache lifecycle events (creation, removal, update or retrieval of entries) 
 - using _EntryProcessor_s to provide composite atomic operations over mutable representations of Cache entries
