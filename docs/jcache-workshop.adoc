...
* Use the `JPACommentsManager` and `JPACommentsManager` inside the JCache impementation to pass through the data to and from the database, when it's not in the cache
* Change the qualifier in the controllers so that they use the JCache implementation instead of the JPA one
* You may consider creating a CDI producer that itself injects the `CacheManager` and creates the actual cache in a `@Produces` method
...
You can create the cache in your JCache managers implementations.
But as we are running in a Java EE application server (involving a CDI container), we'll do that in a separate class, which will act as a producer.
We'll create a `javax.cache.Cache` instance there and we will inject it in our `JCacheCommentsManager` implementation.
To do so create the `CommentsCacheProducer` cache provider class in the `bg.jug.guestbook.comment` package as follows:
...
Now you can inject the comments cache in the `JCacheCommentsManager` class.
...
Now implementat the methods of the `CommentsManager` interface using JCache.
Let's start with getting all the comments.
...
We'll create the producer class in the `bg.jug.guestbook.users` package as follows:
...
And then we'll implement the UserManager interface with JCache in mind:
...
And of course changet the `UserManager` implementation to `@JCache` in the three controllers: `LoginController`, `RegisterController` and `UserController`:
